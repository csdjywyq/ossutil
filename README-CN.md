...
### [README of English](https://github.com/csdjywyq/ossutil/blob/master/README.md)
...
- 首先配置您的go工程目录。
- go get该工程依赖的库。
- go get github.com/csdjywyq/ossutil。
- 进入go工程目录下的src目录，build代码生成ossutil工具，例如：在linux下可以运行go build github.com/csdjywyq/ossutil/ossutil.go。
- 参考上面示例运行ossutil工具。

### 测试
- 进入go工程目录下的src目录，修改github.com/csdjywyq/ossutil/lib/command_test.go里的endpoint、AccessKeyId、AccessKeySecret、STSToken等配置。
- 请在lib目录下执行`go test`。
